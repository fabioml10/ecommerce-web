...
const ProductSearchService = {
  execute({ 
    ...
    if (search) returnStr += `&search=${search}`
    if (page) returnStr += `&page=${page}`
    if (category) returnStr += `&category_ids[]=${category}`

    if (price) {
      let prices = price.toString().split('-')
      returnStr += `&price[min]=${prices[0]}`
      if (prices.length === 2) returnStr += `&price[max]=${prices[1]}`
    }
